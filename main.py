...
def powiaty_load():
    ...
    if db:
        ...
        if res:  # Team posiada powiaty
            ...
            powiat_i = db_attr_check("t_active_pow")  # Pobranie atrybutu t_active_pow z db
...
def powiaty_cb_changed():
    ...
    if db_attr_change(tbl="team_users", attr="t_active_pow", val=t_powiat_i, sql_bns=" AND team_id = " + str(team_i)):
        powiat_t = t_powiat_t
        powiat_i = t_powiat_i
...
def vn_mode_changed(clicked):
    ...
    if user_has_vn():  # Użytkownik ma przydzielone vn'y w aktywnym teamie
        vn_set_gvars(user_id, team_i, dlg.p_pow.is_active(), False)  # Ustalenie parametrów aktywnego vn'a
    else:  # Użytkownik nie ma przydzielonych vn w aktywnym teamie
        vn_set_gvars(user_id, team_i, dlg.p_pow.is_active(), True)  # Ustalenie parametrów aktywnego vn'a
