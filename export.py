...
class ExportCanvasPanel(QFrame):
    ...
    def empty_date_check(self):
        ...
        db = PgConn()
        sql = f"SELECT d.wyr_id, concat(p.pow_id, '_', p.order_id) as id_punkt FROM team_{dlg.team_i}.wyr_dane AS d INNER JOIN team_{dlg.team_i}.wyr_prg AS p ON d.wyr_id=p.wyr_id WHERE p.pow_grp = '{dlg.powiat_i}' and p.order_id IS NOT NULL AND d.date_fchk IS NULL ORDER BY d.wyr_id;"
        if db:
